...
selections_file_path = r'C:\Users\zxm14\Documents\GitHub\Montagesimulation-Copy\pages\selections.json'
...
def main():
    station_number = '1'  # 每个 Arbeitsstation 文件中设置不同的编号
    ...
    if selected_workflow:
        ...
        if data is not None:
            if 'duration_data' not in st.session_state:
                st.session_state.duration_data = []
            ...
            for i, order in enumerate(data, start=1):
                with st.expander(f"Order {i}"):
                    ...
                    if end_button and f'start_time_{station_number}_{i}' in st.session_state:
                        ...
                        st.session_state[f'duration_{station_number}_{i}'] = duration
                        st.session_state.duration_data.append({"Order": i, "Dauerzeit": duration, "Kundentakt": order.get("Kundentakt")})

            # 显示每个订单的 Dauerzeit
            for j in range(1, len(data) + 1):
                if f'duration_{station_number}_{j}' in st.session_state:
                    st.markdown(f"Dauerzeit für Order {j}: <span style='color: red;'>{st.session_state[f'duration_{station_number}_{j}']} Sekunden</span>", unsafe_allow_html=True)

            # 保存持续时间数据到 JSON 文件
            if st.button("Daten speichern"):
                save_duration_file = os.path.join('pages', f'Arbeitsstation{station_number}_durations.json')
                save_duration_data(st.session_state.duration_data, save_duration_file)
